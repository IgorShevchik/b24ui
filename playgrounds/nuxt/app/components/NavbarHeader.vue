...
const componentName = computed(() => normalizePath(route.path).split('/').filter(Boolean).pop() ?? '')
...
const index = computed(() => components?.findIndex(component => normalizePath(component.to) === normalizePath(route.path)) ?? -1)
